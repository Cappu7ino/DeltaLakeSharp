...
- Uses a native async operation `SafeHandle` for V3 partition planning and JSON-result table operations.
...
`GetReadPartitionsAsync`, table creation, protocol upgrade, and SQL DML operations start callback-notified native operations on the shared Tokio runtime. The managed backend awaits a `TaskCompletionSource`, takes the JSON result once after native completion is signaled, and cancels the native operation if the managed cancellation token is signaled. The public API and result models are unchanged.
