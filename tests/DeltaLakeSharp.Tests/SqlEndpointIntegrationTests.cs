...
namespace DeltaLakeSharp.Tests
{
    ...
    public class SqlEndpointIntegrationTests
    {
        ...
        private const string ReadTestTableNameEnvironmentVariable = "DELTALAKESHARP_SQL_ENDPOINT_READ_TABLE";
        ...
        private static string _readTestTableName;

        private static string ReadTestTableName => _readTestTableName;
        ...
        public static void ClassInitialize(TestContext context)
        {
            ...
            _readTestTableName = null;

            try
            {
                string sqlEndpointServer = Environment.GetEnvironmentVariable(SqlEndpointServerEnvironmentVariable);
                string sqlEndpointDatabase = Environment.GetEnvironmentVariable(SqlEndpointDatabaseEnvironmentVariable);
                string readTestTableName = Environment.GetEnvironmentVariable(ReadTestTableNameEnvironmentVariable);

                if (string.IsNullOrWhiteSpace(sqlEndpointServer) ||
                    string.IsNullOrWhiteSpace(sqlEndpointDatabase) ||
                    string.IsNullOrWhiteSpace(readTestTableName))
                {
                    _initFailureReason =
                        "SQL analytics endpoint not configured. " +
                        $"Set {SqlEndpointServerEnvironmentVariable}, {SqlEndpointDatabaseEnvironmentVariable}, and {ReadTestTableNameEnvironmentVariable}.";
                    return;
                }

                _readTestTableName = readTestTableName;
