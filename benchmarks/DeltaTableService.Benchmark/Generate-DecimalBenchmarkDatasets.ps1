param(
    [string]$Project = "benchmarks/DeltaTableService.Benchmark/DeltaTableService.Benchmark.csproj",
    [switch]$Overwrite
)
...
foreach ($dataset in $snapshotDatasets) {
    $output = "benchmarks/DeltaTableService.Benchmark/TestData/delta-full-read-decimal/$($dataset.Size)"
    $args = @(
        'run',
        '--project', $Project,
        '--framework', 'net8.0',
...
$cdfOutput = 'benchmarks/DeltaTableService.Benchmark/TestData/delta-full-cdf-decimal'
$cdfArgs = @(
    'run',
    '--project', $Project,
    '--framework', 'net8.0',
